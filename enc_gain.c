                             ...
                             Word32 corr_offset)
{
   ...
   for (t = t_min; t <= t_max; t++)
   {
      ...
      corr_norm[corr_offset + t] = ps * norm;
                             ...
                             Word32 i_subfr, Word32 t0_fr2, Word32 t0_fr1)
{
   ...
   E_GAIN_norm_corr(exc, xn, h, t_min, t_max, corr_v, corr_v_offset);
   ...
   cor_max = E_GAIN_norm_corr_interpolate(&corr_v[corr_v_offset + t0], fraction);

   for (i = (fraction + step); i <= 3; i += step)
   {
      temp = E_GAIN_norm_corr_interpolate(&corr_v[corr_v_offset + t0], i);
